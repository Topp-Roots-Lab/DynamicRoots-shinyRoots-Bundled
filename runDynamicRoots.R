dir.name <- dirname(rstudioapi::getSourceEditorContext()$path)
setwd(dir.name)
...
timestamp <-
  strftime(as.character(Sys.time()), format = "%Y-%m-%d_%H-%M-%S")
...
outfiles_path <-
  paste("processedFiles\\", foldername, "\\", sep = "")
resfiles_path <-
  paste("dynamicRootsOutputs\\", foldername, "\\", sep = "")
traitfiles_path <-
  paste("dynamicRootsTraitsFiles\\", foldername, "\\", sep = "")
...
originalfilenames <-
  list.files(path = inputfiles_path,
             pattern = ".out",
             full.names = TRUE)
...
if (nfiles != 0)
{
  for (i in 1:length(originalfilenames))
  {
    ...
    filename <-
      paste(outfiles_path, basename(originalfilenames[i]), sep = "")
    ...
    resfiles_pathx <-
      paste(resfiles_path, basename(fnamex), sep = "")
    ...
    command <-
      paste(dynamicRoots, resfiles_pathx, fnamex_obj, fnamex_obj, sep = " ")
    ...
    trait_filename <-
      paste(resfiles_pathx,
            "\\",
            basename(fnamex),
            "_per_branch_dynamics.txt",
            sep = "")
